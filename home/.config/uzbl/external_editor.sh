...
if [ "$value" != "$(< $tmp_file)" ]
    then
    echo "script /home/@user/.config/uzbl/base64.js" > "$UZBL_FIFO"
    # in case that actelem.type has changed, we do this test
    echo "js var actelem = document.activeElement; if(actelem.type == 'text' || actelem.type == 'textarea') {actelem.value = decode64('ase64 $tmp_file | tr -d '\n'fi
